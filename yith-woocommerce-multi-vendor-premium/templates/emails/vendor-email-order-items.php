...
foreach ( $items as $item_id => $item ) :
	...
	<tr class="<?php echo esc_attr( apply_filters( 'woocoomerce_order_item_class', 'order_item', $item, $order ) ); ?>">
		...
		<td style="text-align:center; vertical-align:middle; border: 1px solid #eee;">
			<?php echo ( $commission && $commission->exists() ) ? $commission->get_rate( 'display' ) : '-'; // phpcs:ignore ?>
		</td>
		...
		<?php endif; ?>
		<td style="text-align:left; vertical-align:middle; border: 1px solid #eee;"><?php echo ( $commission && $commission->exists() ) ? $commission->get_amount( 'display' ) : '-'; // phpcs:ignore
		?>
		</td>
	</tr>
	<?php
	if ( $commission && $commission->exists() ) {
		$total_commissions_amount += $commission->get_amount();
	}
