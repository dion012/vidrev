...
if ( ! class_exists( 'YITH_Vendors_Commissions' ) ) {
	...
	class YITH_Vendors_Commissions extends YITH_Vendors_Commissions_Legacy {
		...
		public function get_line_item_total( $item, $order ) {
			...
			$get_item_amount = $this->include_coupon ? 'get_line_total' : 'get_line_subtotal';
		...
		protected function register_commissions_status( $order_id, $status ) {
			...
			foreach ( $commission_ids as $commission_id ) {
				...
				if ( ! $commission || ! $commission->exists() || ( 'woocommerce_order_status_changed' === current_action() && in_array( $commission->get_status(), array( 'paid', 'processing' ), true ) ) ) {
					continue;
				}
		...
		protected function register_commissions_refunded( $order_id ) {
			...
			$order = wc_get_order( $order_id );
			if ( $order && 'yes' !== $order->get_meta( '_commissions_refunded' ) ) {
				if ( $this->register_commissions_status( $order_id, 'refunded' ) ) {
		...
		protected function register_commissions_cancelled( $order_id ) {
			$order = wc_get_order( $order_id );
			if ( $order && 'yes' !== $order->get_meta( '_commissions_cancelled' ) ) {
				if ( $this->register_commissions_status( $order_id, 'cancelled' ) ) {
			...
			foreach ( $items as $refund_item ) {
				...
				if ( $refund_amount > 0 ) {
					...
					if ( $commission && $commission->exists() ) {
						...
						$commission_amount_refunded += (float) $commission->get_amount_refunded();
						$commission->set_amount_refunded( $commission_amount_refunded );
		...
		public static function get_tax_and_coupon_management_message( $commission_included_tax = null, $commission_included_coupon = null ) {
			...
			$coupon = $commission_included_coupon ? _x( 'included', 'means: Vendor commission have been calculated: coupon included', 'yith-woocommerce-product-vendors' ) : _x( 'excluded', 'means: Vendor commission have been calculated: tax excluded', 'yith-woocommerce-product-vendors' );
