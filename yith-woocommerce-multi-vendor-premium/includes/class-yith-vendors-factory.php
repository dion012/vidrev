	...
	class YITH_Vendors_Factory {
				...
				switch ( $object_type ) {
					...
					case 'vendor':
						if ( is_numeric( $object ) ) {
							$vendor_id = absint( $object );
						} elseif ( is_string( $object ) ) {
							// Try to get vendor by slug.
							$vendor_term = get_term_by( 'slug', $object, YITH_Vendors_Taxonomy::TAXONOMY_NAME );
							if ( empty( $vendor_term ) || is_wp_error( $vendor_term ) ) {
								// Try to get vendor by name.
								$vendor_term = get_term_by( 'name', $object, YITH_Vendors_Taxonomy::TAXONOMY_NAME );
							}

							if ( ! empty( $vendor_term ) && ! is_wp_error( $vendor_term ) ) {
								$vendor_id = $vendor_term->term_id;
							}
						} elseif ( $object instanceof YITH_Vendor ) {
		...
		protected static function instance( $vendor_id = 0 ) {
			...
			try {
				...
				if ( empty( self::$instances ) || ! isset( self::$instances[ $vendor_id ] ) ) {
					$vendor                        = new YITH_Vendor( $vendor_id );
					self::$instances[ $vendor_id ] = $vendor;
				}
		...
		public static function get_data( $vendor_id, $modal = true, $fields = array() ) {
			...
			try {
				...
				return apply_filters( 'yith_wcmv_get_vendor_data', $data, $vendor, $modal, $fields );
		...
		public static function query( $args ) {
			...
			if ( '' !== $args['enabled_selling'] ) {
				$query_args['meta_query'][] = array(
					'key'   => 'enable_selling',
					'value' => ( true === $args['enabled_selling'] || 'yes' === $args['enabled_selling'] ) ? 'yes' : 'no',
				);
			}
			...
			if ( ! empty( $args['pending'] ) ) {
				$query_args['meta_query'][] = array(
					'key'   => 'pending',
					'value' => ( true === $args['pending'] || 'yes' === $args['pending'] ) ? 'yes' : 'no',
				);
			}
		 ...
		 * @since  4.0.0
		 * @author Francesco Licandro
		 * @param boolean $plan (Optional) True to get a plan array, false otherwise. Default false.
		 * @return array
		 */
		public static function get_fields( $plan = false ) {
			global $wp_rewrite;
			...
			$fields = array(
				...
				'store'   => array(
					...
					'header_image'  => array(
						'type'  => 'header_image',
						'icon'  => 'store',
						'label' => _x( 'Upload Vendor header image (Size 1400x460px)', '[Admin] Vendor option label', 'yith-woocommerce-product-vendors' ),
					),
					'field_sep'     => array(
						'type' => 'separator',
					),
			...
			$additional_fields = yith_wcmv_get_vendor_registration_fields();
			...
			$fields = apply_filters( 'yith_wcmv_vendor_admin_fields', $fields, $plan );
