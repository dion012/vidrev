...
if ( ! class_exists( 'YITH_Vendors_Shipping_Frontend' ) ) {
	...
	class YITH_Vendors_Shipping_Frontend {
		...
		public function woocommerce_cart_shipping_packages( $packages ) {
			...
			foreach ( $vendors as $vendor ) {
				if ( YITH_Vendors_Shipping::is_single_vendor_shipping_enabled( $vendor ) ) {
					$matched_zone = $this->get_matched_zone( $vendor );

					if ( false !== $matched_zone ) {
						$packages[] = $this->get_package( $vendor, $matched_zone );
					}
				}
			}
		 ...
		 * @param integer     $zone_key The vendor zone key.
		 * @return array
		 */
		protected function get_package( $vendor, $zone_key ) {
			return $this->calculate_package_cost(
				array(
					...
					'yith-vendor'                   => $vendor,
					'yith-vendor-shipping-zone-key' => $zone_key,
				)
			);
		}
		 ...
		 * @return array
		 */
		protected function get_vendors_cart_contens( $vendor ) {
		...
		public function woocommerce_shipping_package_name( $title, $i, $package ) {
			if ( isset( $package['yith-vendor-shipping-zone-key'] ) && isset( $package['yith-vendor'] ) ) {
				$title = apply_filters( 'yith_wcmv_shipping_package_name', $package['yith-vendor']->get_name(), $package['yith-vendor'], $i );
			}
		...
		public function woocommerce_package_rates( $rates, $package ) {
			...
			$zone_key = $package['yith-vendor-shipping-zone-key'];
		...
		protected function calculate_shipping_method_rate( $package, $key, $shipping_method, &$rates ) {
			...
			foreach ( $package['contents'] as $sc_id => $sc_args ) {
				if ( isset( $sc_args['quantity'] ) ) {
					$total_qty += $sc_args['quantity'];
				}
			}
			...
			if ( empty( $shipping_method['method_cost'] ) ) {
				...
				$shipping_method['method_cost'] = class_exists( 'WC_Eval_Math' ) ? WC_Eval_Math::evaluate( do_shortcode( str_replace( '[qty]', $total_qty, $shipping_method['method_cost'] ) ) ) : $shipping_method['method_cost'];
			}
		...
		protected function get_extra_cost( $package, $shipping_method ) {
			...
			$extra_cost = 0;
			if ( 'no' === $vendor->get_meta( 'enable_shipping_extra_cost' ) ) {
				return $extra_cost;
			}
			...
			$extra_cost += $this->get_extra_cost_rule( $vendor->get_meta( 'shipping_extra_cost_items' ), count( $items ) );
			...
			foreach ( $items as $item ) {
				...
				$product     = $item['data'];
				$extra_cost += $this->get_extra_cost_rule( $vendor->get_meta( 'shipping_extra_cost_products' ), $item['quantity'] );
				...
				} elseif ( null === term_exists( $product_shipping_class_id, 'product_shipping_class' ) ) {
				...
				if ( ! empty( $product_shipping_class_id ) && isset( $shipping_method[ $product_shipping_class_id ] ) ) {
					...
					$shipping_class_costs[ $product_shipping_class_id ]['qty']       += $item['quantity'];
					$shipping_class_costs[ $product_shipping_class_id ]['line_total'] = $item['line_total'];
				}
			}
			...
			foreach ( $shipping_class_costs as $sc_id => $sc_args ) {
				...
				$sc_cost = wc_format_decimal( $sc_cost );
