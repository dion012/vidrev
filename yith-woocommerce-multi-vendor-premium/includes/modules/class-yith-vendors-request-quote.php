...
if ( ! class_exists( 'YITH_Vendors_Request_Quote' ) && function_exists( 'YITH_Request_Quote' ) ) {
	...
	class YITH_Vendors_Request_Quote {
		...
		private function __construct() {
			...
			// add_filter( 'woocommerce_order_get_total', array( $this, 'quote_get_total' ), 10, 3 ); Filter temporary removed to fix list table price issue.
		 ...
		 * @return YITH_Vendors_Request_Quote Main instance
		 */
		...
		public function check_if_quote_have_commissions() {
			if ( ! empty( $_POST['order_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				$order_id = absint( $_POST['order_id'] ); // phpcs:ignore WordPress.Security.NonceVerification
				if ( $this->has_valid_quote_status( $order_id ) && wp_get_post_parent_id( $order_id ) ) {
		 ...
		 *
		 * @param float    $value Original value fro main quote.
		...
		public function quote_get_total( $value, $order ) {
			...
			if ( 'edit.php' === $pagenow && ! empty( $_GET['post_type'] ) && 'shop_order' === sanitize_text_field( wp_unslash( $_GET['post_type'] ) ) && function_exists( 'YITH_YWRAQ_Order_Request' ) && YITH_YWRAQ_Order_Request()->is_quote( $order->get_id() ) && ! $order->get_parent_id() ) { // phpcs:ignore WordPress.Security.NonceVerification
