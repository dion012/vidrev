...
if ( ! class_exists( 'YITH_Report_Sale_Commissions' ) ) {
	...
	class YITH_Report_Sale_Commissions extends WC_Admin_Report {
		...
		private function query_report_data() {
			...
			$query_args = array_merge( array( 'vendor_id' => $vendor->get_id() ), $date_query );

			$commission_status        = YITH_Vendors()->commissions->get_status();
			...
			foreach ( $commission_status as $key => $value ) {
				...
				foreach ( $commission_ids as $commission_id ) {
					...
					if ( 'all' === $key && ( 'cancelled' === $commission->get_status() || 'refunded' === $commission->get_status() ) ) {
						continue;
					}
						...
						var series = [
							{
								...
								bars: { fillColor: '<?php echo esc_attr( $this->chart_colours['commissions_count'] ); ?>', fill: true, show: true, lineWidth: 0, barWidth: <?php echo esc_attr( $this->barwidth ); ?> * 0.5, align: 'center' },
								shadowSize: 0,
								hoverable: true,
