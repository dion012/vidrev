<?php
/**
 * YITH_WCACT_Email_New_Bid Class.
 *
...
/**
 *  Email new notification for bidders and followes.
 *
 ...
 */
if ( ! class_exists( 'YITH_WCACT_Email_New_Bid' ) ) {
	/**
	 * Class YITH_WCACT_Email_New_Bid
	 *
	 * @author Carlos Rodríguez <carlos.rodriguez@yithemes.com>
	 */
	class YITH_WCACT_Email_New_Bid extends WC_Email {
		...
		public function __construct() {
			...
			$this->id = 'yith_wcact_email_new_bid';

			// this is the title in WooCommerce Email settings.
			$this->title = esc_html__( 'Auctions - New bid for bidders and followers', 'yith-auctions-for-woocommerce' );

			$this->customer_email = true;

			// this is the description in WooCommerce email settings.
			$this->description = esc_html__( 'Email sent to followers and bidders when the auction has a new bid.', 'yith-auctions-for-woocommerce' );

			// these are the default heading and subject lines that can be overridden using the settings.
			$this->heading = esc_html__( 'There is a new bid for the item', 'yith-auctions-for-woocommerce' );
			$this->subject = esc_html__( 'There is a new bid for the item', 'yith-auctions-for-woocommerce' );

			// these define the locations of the templates that this email should use, we'll just use the new order template since this email is similar.
			$this->template_html = 'emails/common/new-bid.php';

			// Trigger on new bid.
			add_action( 'yith_wcact_email_new_bid', array( $this, 'trigger' ), 10, 3 );
		 ...
		 * @return string
		 */
		public function trigger( $user_id_or_mail, $product_id, $args ) {
			/*
			...
			if ( $product && 'auction' === $product->get_type() ) {
				...
				$this->object = array(
					...
					'hash'         => isset( $args['follower'] ) ? $args['follower']->hash : '',
				);
				$this->send(
		...
		public function is_enabled() {
			...
			if ( 'yes' === get_option( 'yith_wcact_email_bidders_new_bid', 'no' ) || 'yes' === get_option( 'yith_wcact_notify_followers_on_new_bids', 'no' ) ) {
				$enabled = true;
			}
...
return new YITH_WCACT_Email_New_Bid();
