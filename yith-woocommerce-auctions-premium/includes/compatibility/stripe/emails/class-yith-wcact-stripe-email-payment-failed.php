<?php
/**
 * YITH_WCACT_Stripe_Email_Payment_Failed Class.
 *
...
/**
 * Class Stripe couldn't process payment ( second attempt )
 *
 * @class   YITH_WCACT_Stripe_Email_Payment_Failed
 * @package Yithemes
 * @since   Version 3.0.0
 * @author  Carlos Rodríguez <carlos.rodriguez@yithemes.com>
 */
if ( ! class_exists( 'YITH_WCACT_Stripe_Email_Payment_Failed' ) ) {
	/**
	 * Class YITH_WCACT_Stripe_Email_Payment_Failed
	 *
	 * @author Carlos Rodríguez <carlos.rodriguez@yithemes.com>
	 */
	class YITH_WCACT_Stripe_Email_Payment_Failed extends WC_Email {
		...
		public function __construct() {
			$this->id = 'yith_wcact_stripe_email_payment_failed';

			$this->title = esc_html__( 'Auctions - Stripe payment failed', 'yith-auctions-for-woocommerce' );

			$this->description = esc_html__( 'Email sent when Stripe cannot process the second payment attempt.', 'yith-auctions-for-woocommerce' );

			$this->heading = esc_html__( 'Payment failed - Don\'t lose your item', 'yith-auctions-for-woocommerce' );
			$this->subject = esc_html__( '[{site_title}] - Payment failed - Don\'t lose your item', 'yith-auctions-for-woocommerce' );

			$this->template_html = 'emails/stripe/payment-failed.php';

			$this->customer_email = true;

			add_action( 'yith_wcact_failed_attempts_2', array( $this, 'trigger' ), 10, 3 );
		...
		public function trigger( $order, $product, $gateway_response ) {
			...
			ywcact_logs( 'Sent email attempt 2 for product ' . $product->get_id() . ' on order ' . $order->get_id() );
		}
...
return new YITH_WCACT_Stripe_Email_Payment_Failed();
